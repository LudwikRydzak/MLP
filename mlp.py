...
import numpy as np
class mlp:
    def __init__(self, _layers_size, _weight_range, _bias_range, _activation_functions ):
    ...
    def activation_function(self, pobudzenie, layer):
        ...
        for i in pobudzenie:
            if(self.activation_functions[layer]==1):
                ...
                ret.append(self.activation_sigmoidal(i))
    ...
    def forward_propagation(self, _entry_values):
        for layer_number in range(len(self.layers_size)-1):
            self.pobudzenia[layer_number] = self.pobudzenie(self.weights[layer_number], _entry_values, self.biases[layer_number])
            self.wyjscia[layer_number]= self.activation_function(self.pobudzenia[layer_number],layer_number)
        return self.softMax(self.wyjscia[len(self.layers_size)-1])